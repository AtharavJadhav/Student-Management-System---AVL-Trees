...
Tree_Node* make_tree(Tree_Node* root)
{
    ...
    int x;
    errno_t error_code;
    error_code = fopen_s(&fp, "C:/Users/Atharav Jadhav/source/repos/Project with Avl/Input.txt", "r");
    if (error_code != 0)
    {
...
Tree_Node* delete_roll(Tree_Node* root, Tree_Node* newRoot, char* roll)
{
    if (root == NULL)
    {
        return newRoot;
    }
    newRoot = delete_roll(root->left, newRoot, roll);
    if (strcmp(root->data.Roll_No, roll) != 0)
    {
        newRoot = insertNode(newRoot, root->data);
    }
    newRoot = delete_roll(root->right, newRoot, roll);
    return newRoot;
}
...
int main()
{
    ...
    Del_Roll = delete_roll(Root, Del_Roll, str2);
    */
    ...
    scanf_s("%s", &str2);
    Range = rangeSort(Root, Range, str1, str2);
    
    
    printf("After Execution:\n");
    printInOrder(Range);

    delete_tree(Del_Roll);
    delete_tree(Root);
    delete_tree(root);
